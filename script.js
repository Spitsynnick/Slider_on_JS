let sliderItems = [ // массив данных: 3 объекта с фото и текстом, которые меняются при переключении слайдера
...
function initSlider() { // функция, отвечающая за представление слайдера
    ...
    function initImages() { // функция создает div с фоном (фото из массива), назначает ему классы и добавляет его в разметку
    ...
    function initArrows() { // функция отвечает за стрелки в слайдере: назначает им обработчики нажатия в зависимости от класса (левая - класс arrow_left, правая - класс arrow_right)
    ...
    function initDots() { // функция создает точки в слайдере (класс dot) по количеству фото в массиве, назначает им классы и добавляет в разметку
        sliderItems.forEach((sliderItem, index) => {
    ...
    function initText() { // функция назначает текст для первого слайда всем соответствующим блокам при загрузке страницы          
    ...
    function initLinks() { // функция отвечает за ссылки над слайдером: при загрузке назначает класс activeLink первой ссылке, затем назначает им обработчики нажатия, переключающие слайдер
    ...
    function moveSlider(number) { // функция отвечает за переключение слайда, отображение активной точки, актывной ссылки, а также текста из массива, соответствующего слайду       
